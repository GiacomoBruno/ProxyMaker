    ...
    for (auto const &p : images)
    {
        std::ignore = pool.submit_task(
            [&conf, p, out, fullart]()
            {
                ...
                if (fullart)
                    toCrop -= 0.4; // for some reason full art images from mpcfill have a bigger bleed it seems.
                ...
                {
                    cv::Mat RotatedImage;
                    cv::rotate(cropped_image, RotatedImage, cv::ROTATE_90_CLOCKWISE);
                    cv::imwrite(cropped.string(), RotatedImage);
                }
                else
                {
...
void PadImages(Configuration const &conf, path const &input, path const &output)
{
    ...
    for (auto image : LoadImages(input, true))
    {
        std::ignore = pool.submit_task([image, &conf, &output]()
                                       {
                                           if (std::filesystem::exists(output.string() + std::filesystem::path(image).filename().string()))
                                               return;
